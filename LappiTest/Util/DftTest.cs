...
namespace LappiTest.Util {
    ...
    public class DftTest {
        ...
        private readonly Dft dft = new Dft();

        /// <summary>
        ///     Second example from http://www.sccon.ca/sccon/Dft/Dft3.htm
        /// </summary>
        [Test]
        public void Simple_example () {
            ...
            Assert.That(dft.Forward(time), Is.EqualTo(freq).Using(Complex.Within(1E-6)));
        }
        ...
        private void AssertTimeFreq (Complex[] time, Complex[] freq) {
            Assert.That(dft.Forward(time), Is.EqualTo(freq).Using(Complex.Within(1E-16)));
            Assert.That(dft.Inverse(freq), Is.EqualTo(time).Using(Complex.Within(1E-16)));
        }
